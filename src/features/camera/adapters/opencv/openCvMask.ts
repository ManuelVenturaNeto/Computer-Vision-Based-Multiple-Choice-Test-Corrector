...
export function buildMaskImage(
  ...
  cells: AnswerCell[],
  detectedAlts: number[],
  grid: { rows: number; cols: number; totalRows: number; totalCols: number; startX: number; startY: number; cellWidth: number; cellHeight: number }
) {
  ...
  for (let row = 0; row <= grid.totalRows; row += 1) cv.line(warpedResult, new cv.Point(grid.startX, grid.startY + row * grid.cellHeight), new cv.Point(grid.startX + grid.totalCols * grid.cellWidth, grid.startY + row * grid.cellHeight), purple, 2);
  for (let col = 0; col <= grid.totalCols; col += 1) cv.line(warpedResult, new cv.Point(grid.startX + col * grid.cellWidth, grid.startY), new cv.Point(grid.startX + col * grid.cellWidth, grid.startY + grid.totalRows * grid.cellHeight), purple, 2);
